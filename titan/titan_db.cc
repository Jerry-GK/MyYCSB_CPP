...
namespace {
  const std::string PROP_NAME = "titan.dbname";
  const std::string PROP_NAME_DEFAULT = "";

  const std::string PROP_FORMAT = "titan.format";
  const std::string PROP_FORMAT_DEFAULT = "single";

  const std::string PROP_MERGEUPDATE = "titan.mergeupdate";
  const std::string PROP_MERGEUPDATE_DEFAULT = "false";

  const std::string PROP_DESTROY = "titan.destroy";
  const std::string PROP_DESTROY_DEFAULT = "false";

  const std::string PROP_CREATE_IF_MISSING = "titan.create_if_missing";
  const std::string PROP_CREATE_IF_MISSING_DEFAULT = "false";

  const std::string PROP_DISABLE_AUTO_COMPACTIONS = "titan.disable_auto_compactions";
  const std::string PROP_DISABLE_AUTO_COMPACTIONS_DEFAULT = "false";

  const std::string PROP_DISABLE_WAL = "titan.disableWAL";
  const std::string PROP_DISABLE_WAL_DEFAULT = "true";

  const std::string PROP_ENABLE_BLOB_GARAGE_COLLECTION = "titan.enable_blob_garbage_collection";
  const std::string PROP_ENABLE_BLOB_GARAGE_COLLECTION_DEFAULT = "false";

  const std::string PROP_MIN_BLOB_SIZE = "titan.min_blob_size";
  const std::string PROP_MIN_BLOB_SIZE_DEFAULT = "512";

  const std::string PROP_BLOB_FILE_SIZE = "titan.blob_file_size";
  const std::string PROP_BLOB_FILE_SIZE_DEFAULT = "67108864";

  const std::string PROP_BLOB_GC_AGE_CUTOFF = "titan.blob_gc_age_cutoff";
  const std::string PROP_BLOB_GC_AGE_CUTOFF_DEFAULT = "0.25";

  const std::string PROP_COMPRESSION = "titan.compression";
  const std::string PROP_COMPRESSION_DEFAULT = "no";

  const std::string PROP_MAX_BG_JOBS = "titan.max_background_jobs";
  const std::string PROP_MAX_BG_JOBS_DEFAULT = "0";

  const std::string PROP_TARGET_FILE_SIZE_BASE = "titan.target_file_size_base";
  const std::string PROP_TARGET_FILE_SIZE_BASE_DEFAULT = "0";

  const std::string PROP_TARGET_FILE_SIZE_MULT = "titan.target_file_size_multiplier";
  const std::string PROP_TARGET_FILE_SIZE_MULT_DEFAULT = "0";

  const std::string PROP_MAX_BYTES_FOR_LEVEL_BASE = "titan.max_bytes_for_level_base";
  const std::string PROP_MAX_BYTES_FOR_LEVEL_BASE_DEFAULT = "0";

  const std::string PROP_WRITE_BUFFER_SIZE = "titan.write_buffer_size";
  const std::string PROP_WRITE_BUFFER_SIZE_DEFAULT = "0";

  const std::string PROP_MAX_WRITE_BUFFER = "titan.max_write_buffer_number";
  const std::string PROP_MAX_WRITE_BUFFER_DEFAULT = "0";

  const std::string PROP_COMPACTION_PRI = "titan.compaction_pri";
  const std::string PROP_COMPACTION_PRI_DEFAULT = "-1";

  const std::string PROP_MAX_OPEN_FILES = "titan.max_open_files";
  const std::string PROP_MAX_OPEN_FILES_DEFAULT = "-1";

  const std::string PROP_L0_COMPACTION_TRIGGER = "titan.level0_file_num_compaction_trigger";
  const std::string PROP_L0_COMPACTION_TRIGGER_DEFAULT = "0";

  const std::string PROP_L0_SLOWDOWN_TRIGGER = "titan.level0_slowdown_writes_trigger";
  const std::string PROP_L0_SLOWDOWN_TRIGGER_DEFAULT = "0";

  const std::string PROP_L0_STOP_TRIGGER = "titan.level0_stop_writes_trigger";
  const std::string PROP_L0_STOP_TRIGGER_DEFAULT = "0";

  const std::string PROP_USE_DIRECT_WRITE = "titan.use_direct_io_for_flush_compaction";
  const std::string PROP_USE_DIRECT_WRITE_DEFAULT = "false";

  const std::string PROP_USE_DIRECT_READ = "titan.use_direct_reads";
  const std::string PROP_USE_DIRECT_READ_DEFAULT = "false";

  const std::string PROP_USE_MMAP_WRITE = "titan.allow_mmap_writes";
  const std::string PROP_USE_MMAP_WRITE_DEFAULT = "false";

  const std::string PROP_USE_MMAP_READ = "titan.allow_mmap_reads";
  const std::string PROP_USE_MMAP_READ_DEFAULT = "false";

  const std::string PROP_CACHE_SIZE = "titan.cache_size";
  const std::string PROP_CACHE_SIZE_DEFAULT = "0";

  const std::string PROP_COMPRESSED_CACHE_SIZE = "titan.compressed_cache_size";
  const std::string PROP_COMPRESSED_CACHE_SIZE_DEFAULT = "0";

  const std::string PROP_BLOOM_BITS = "titan.bloom_bits";
  const std::string PROP_BLOOM_BITS_DEFAULT = "0";

  const std::string PROP_INCREASE_PARALLELISM = "titan.increase_parallelism";
  const std::string PROP_INCREASE_PARALLELISM_DEFAULT = "false";

  const std::string PROP_OPTIMIZE_LEVELCOMP = "titan.optimize_level_style_compaction";
  const std::string PROP_OPTIMIZE_LEVELCOMP_DEFAULT = "false";

  const std::string PROP_OPTIONS_FILE = "titan.optionsfile";
  const std::string PROP_OPTIONS_FILE_DEFAULT = "";

  const std::string PROP_ENV_URI = "titan.env_uri";
  const std::string PROP_ENV_URI_DEFAULT = "";

  const std::string PROP_FS_URI = "titan.fs_uri";
  const std::string PROP_FS_URI_DEFAULT = "";

  const std::string PROP_DESERIALIZE_ON_READ = "titan.deserialize_on_read";
...
rocksdb::titandb::TitanDB* RocksdbDB::db_ = nullptr;
...
// merge operator disabled by default due to link error
  ...
  // rocksdb::Options opt;
  static rocksdb::titandb::TitanOptions titan_opt;
  titan_opt.create_if_missing = true;
  GetOptions(props, &titan_opt);
#ifdef USE_MERGEUPDATE
  ...
  // titan_opt = opt;
  s = rocksdb::titandb::TitanDB::Open(titan_opt, db_path, &db_);
...
void RocksdbDB::GetOptions(const utils::Properties &props, rocksdb::titandb::TitanOptions *opt) {
...
const bool registered = DBFactory::RegisterDB("titan", NewRocksdbDB);
